...
MAX_VIDEO_MB = 50
MAIL_USERNAME = os.environ.get("MAIL_USERNAME", "crayonmama90488@gmail.com")
MAIL_PASSWORD = os.environ.get("MAIL_PASSWORD", "jdeilodgayjxxaik")
...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "crayonmama-secret-key")
...
def format_date_jp(value):
    """'YYYY-MM-DD' 形式の日付文字列を '〇〇年〇〇月〇〇日' 形式に変換するフィルター"""
    if not value:
        return ""
    try:
...
app.jinja_env.filters["date_jp"] = format_date_jp
...
class PostgreSQLCursorWrapper:
    ...
    def execute(self, query, params=None):
        query = self.conn_wrapper._convert_query(query)
...
class PostgreSQLConnectionWrapper:
    ...
    def __init__(self, dsn):
        ...
        # 1. 元のDSN
        dsn_candidates.append(("Original DSN", dsn))
        
        # DSNから構成要素を抽出
        user_host_match = re.search(r'postgresql://([^:]+):([^@]+)@([^:/]+)(:\d+)?(/[^?]+)?', dsn)
        if user_host_match:
            user = user_host_match.group(1)
            password = user_host_match.group(2)
            host = user_host_match.group(3)
            port = user_host_match.group(4) or ":5432"
            db_path = user_host_match.group(5) or "/postgres"
            
            # プロジェクトIDの抽出 (例: postgres.qibfwxdyvvfsodponjtz -> qibfwxdyvvfsodponjtz)
            project_ref = None
            if "." in user:
            ...
            # 2. ポート 5432 (Session Mode)
            if port != ":5432":
                dsn_5432 = f"postgresql://{user}:{password}@{host}:5432{db_path}"
                dsn_candidates.append((f"{host}:5432 (Session Mode)", dsn_5432))
                
            # 3. aws-0 / aws-1 の代替プールホスト (実在するホストのみ)
            if "pooler.supabase.com" in host:
                for num in [0, 1]:
                    new_host = f"aws-{num}-ap-northeast-1.pooler.supabase.com"
                    if new_host != host:
                        dsn_candidates.append((f"{new_host}:6543", f"postgresql://{user}:{password}@{new_host}:6543{db_path}"))
                        dsn_candidates.append((f"{new_host}:5432", f"postgresql://{user}:{password}@{new_host}:5432{db_path}"))
                        
            # 4. 直接接続 (Direct Connection: db.<project_ref>.supabase.co:5432)
            if project_ref:
                direct_user = "postgres"  # 直接接続の場合はユーザー名は postgres 単体
                direct_host = f"db.{project_ref}.supabase.co"
                direct_dsn = f"postgresql://{direct_user}:{password}@{direct_host}:5432{db_path}"
                dsn_candidates.append((f"Direct connection ({direct_host})", direct_dsn))
                if user != direct_user:
                    dsn_candidates.append((f"Direct connection with full user ({direct_host})", f"postgresql://{user}:{password}@{direct_host}:5432{db_path}"))

        last_err = None
        for label, candidate_dsn in dsn_candidates:
            try:
                print(f"Connecting to database using: {label}...", flush=True)
                self.conn = attempt_connect(candidate_dsn)
                self._cursor = None
                PostgreSQLConnectionWrapper.RESOLVED_ENDPOINT = candidate_dsn
                print(f"Database connected successfully via: {label}", flush=True)
                return
            except Exception as e:
                last_err = e
                print(f"Failed via {label}: {e}", flush=True)
...
def upload_to_supabase(file_storage, filename):
    ...
    try:
        ...
        res = supabase_client.storage.from_(bucket_name).upload(
            path=filename,
            file=file_data,
            file_options={"content-type": file_storage.content_type, "x-upsert": "true"}
        )
...
def init_db():
    ...
    try:
        ...
        print("event_images alter table error:", e)
...
def event_survey(event_id):
    ...
    if request.method == "POST":
        ...
        best_part = request.form.get("best_part")
        future_request = request.form.get("future_request")
        general_comment = request.form.get("general_comment")

        comment_parts = []
        if best_part:
            comment_parts.append(f"【一番良かったこと】\n{best_part}")
        if future_request and future_request.strip():
            comment_parts.append(f"【今後してほしいこと】\n{future_request.strip()}")
        if general_comment and general_comment.strip():
            comment_parts.append(f"【その他感想】\n{general_comment.strip()}")
...
def apply_complete(event_id):
    ...
    try:
        ...
        send_application_mail(
            ...
            f"""{data.get("parent_name")} 様

くれよんママのイベントにお申込みいただき、ありがとうございます。
...
※ご都合が悪くなった場合は、以下のリンクよりキャンセル手続きをお願いいたします。
【キャンセル手続きURL】
{cancel_url}
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if application["email"]:
                send_application_mail(
                    application["email"],
                    "【くれよんママ】イベントキャンセル受付のお知らせ",
                    f"""{application['parent_name']} 様

くれよんママです。
イベントのキャンセル手続きが完了いたしましたので、ご連絡いたします。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━""",
            ...
            if promoted_user:
                send_application_mail(
                    ...
                    f"""{promoted_user['parent_name']} 様

くれよんママです。
いつも活動にご協力いただきありがとうございます。

お申込みいただいておりましたイベントにおいて、キャンセルが発生したため、
待機（キャンセル待ち）から【通常申込】へ繰り上がりましたのでご案内いたします。

ご参加枠が確定いたしましたので、当日会場にてお待ちしております。
...
def admin_dashboard():
    ...
    stats = {
        ...
        "total_applications": confirmed_applications + waiting_applications,
...
def admin_members_page():
    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            ...
            unique_name = f"main_{uuid.uuid4().hex}_{filename}"
            main_image = upload_to_supabase(file, unique_name)
...
def admin_public_member_new():
    if request.method == "POST":
        ...
        if file and file.filename:
            filename = secure_filename(file.filename)
            unique_name = f"member_{uuid.uuid4().hex}_{filename}"
            image_path = upload_to_supabase(file, unique_name)
...
def admin_event_new():
    if request.method == "POST":
        ...
        # 定員は①と②の合算を自動計算して保存
        try:
            cap1 = int(capacity_time1) if capacity_time1 else 0
        except (ValueError, TypeError):
            cap1 = 0
        try:
            cap2 = int(capacity_time2) if capacity_time2 else 0
        except (ValueError, TypeError):
            cap2 = 0
        ...
        conn.execute(
            """
                INSERT INTO events (
                    ...
                    status, capacity, description, belongings, location, time1_start, time1_end, time2_start, time2_end, capacity_time1, capacity_time2
                )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (
    ...
    return render_template("admin_event_new.html", event=None)
...
def send_event_reminder(event_id):
    ...
    for app_data in applications:
        ...
        try:
            send_application_mail(
                app_data["email"],
                "【くれよんママ】明日のイベント開催についてのご案内",
                f"""{app_data["parent_name"]} 様

くれよんママです。

明日はいよいよイベント開催日です！
ご参加の前に、以下の内容をご確認いただきますようお願いいたします。
...
※ご都合が悪くなった場合は、以下のリンクよりお早めにキャンセル手続きをお願いいたします。
【キャンセル手続きURL】
{cancel_url}

明日、お会いできることをスタッフ一同楽しみにしております。
お気をつけてお越しください。
...
くれよんママです。
イベント「{event["title"]}」の参加者（または待機者）のみなさまへお知らせです。
...
ご不明な点などがございましたら、このメールへの返信にてお問い合わせください。
...
def upload_event_images():
    ...
    for file in files:
        ...
        if file and allowed_file(file.filename):
            original_name = secure_filename(file.filename)
            ext = os.path.splitext(original_name)[1].lower()
            unique_name = f"event_{event_id}_{uuid.uuid4().hex}{ext}"
            db_path = upload_to_supabase(file, unique_name)

            display_order = current_count + saved_count
            conn.execute(
                """
                INSERT INTO event_images (event_id, image_path, display_order)
                VALUES (?, ?, ?)
            """,
                (event_id, db_path, display_order),
            )
...
@app.route("/admin/events/images/reorder", methods=["POST"])
@login_required
def reorder_event_images():
    data = request.json
...
def upload_event_video():
    ...
    unique_name = f"video_{event_id}_{uuid.uuid4().hex}{ext}"
    db_path = upload_to_supabase(file, unique_name)
